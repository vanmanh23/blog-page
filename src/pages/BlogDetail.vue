<template>
  <div v-if="!isLoading" class="flex flex-col gap-5 md:px-[200px] px-5 bg-white dark:bg-gray-800">
    <div class="flex flex-col md:gap-5 gap-2">
      <h2 class="text-primary_15 md:text-[24px] text-[18px] font-bold dark:text-white">{{ BlogData.title }}</h2>
      <p class="text-black dark:text-gray-300 md:text-[12px] text-[12px]">{{ BlogData.text }}</p>
    </div>
    <div class="flex flex-row md:gap-6 gap-1 md:mt-10 mt-3">
      ...
      <div class="flex flex-col gap-3">
        <h2 class="text-primary_25 md:text-[18px] text-[14px] font-semibold dark:text-white">{{ BlogData.heading }}</h2>
        <p class="text-slate-600 dark:text-gray-400 text-[14px]">{{ BlogData.content }}</p>
